...
public class BidPlatformAction extends SupervisorController {
	 ...
	 * 定期标及时放款审核详情
	 */
	public static void regularDetails(long bidId){
			
			if(0 == bidId) render();
			
			Bid bid = new Bid();
			bid.bidDetail = true;
			bid.upNextFlag = Constants.BID_REGULAR;
			bid.id = bidId;
	 ...
	 * 借款中->待放款 
	 */
	public static void fundraiseRegularToEaitLoan(String sign) { 
		checkAuthenticity();
		...
		bid.regularFundraiseToLoan(error);
		flash.error(error.msg); 
	 ...
	 *  借款中->放款审核不通过 
	 */
	public static void fundraiseRegularToLoanNotThrough(String sign) { 
		checkAuthenticity();
	 ...
	 * 上传资料
	 */  
	public static void createUserAuditItem(long userId, long userItemId, String items){
        ErrorInfo error = new ErrorInfo();
