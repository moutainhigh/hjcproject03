...
//		renderArgs.put("ads", ads);
	...
	public static void createBid(Bid bid, String signProductId, String uuid, int status) {
		...
		if(Constants.IPS_ENABLE){					
			...
			//普通网关标的发布的实际业务			
			//标的发布的实际业务逻辑
			bid.afterCreateBid(tbid, null,null, Constants.CLIENT_PC, 0, error);	
			bid.id = tbid.id;
			flash.put("no", OptionKeys.getvalue(OptionKeys.LOAN_NUMBER, error) + bid.getId());
   ...
   /**
	...
	public static void saveInformation(String realityName, int sex, int age,
		...
		user.editInfo(realityName, sex, age, city, idNumber, education, marital, car, house, error);
