...
public class Optimization {
	...
	public static class UserOZ implements Serializable{
		...
		public Double getInvest_amount() {
			if(0 == this.invest_amount){
				String sql = "SELECT SUM(a.amount) AS invest_amount FROM t_invests a,t_bids b WHERE a.bid_id = b.id AND a.transfer_status = 0 and b.`status` IN (?, ?) AND a.user_id = ?";
				Object record = null;
				...
				Query query = em.createNativeQuery(sql).setParameter(1, Constants.BID_REPAYMENT).setParameter(2, Constants.BID_COMPENSATE_REPAYMENT).setParameter(3, this.userId);
	...
	public static class AuditItemOZ {
		...
		public static void createItemStatistic(long userId){
			...
			try {
				...
				Logger.info("查询 资料总数时，%s", e.getMessage());
				return ;
			}
	...
	public static class BidOZ {
    	 ...
    	 * @return
    	 */
    	public static List<AgencyBid> queryAgencyBids(int noPage, PageBean<AgencyBid> pageBean, ErrorInfo error , String ... str){
    	 ...
    	 * @return
    	 */
    	public static List<RegularBid> queryRegularBids(int noPage, PageBean<RegularBid> pageBean, ErrorInfo error , String ... str){
			String[] BID_SEARCH = {" AND (`u`.`name` LIKE ? OR concat(`e`.`_value`,cast(`b`.`id` AS CHAR charset utf8)) LIKE ?)",
					" AND concat(`e`.`_value`,cast(`b`.`id` AS CHAR charset utf8)) LIKE ?", "", " AND `u`.`name` LIKE ?" };
    		...
    		try {
				...
				Logger.error("借款标管理->定期标列表,查询总记录时：" + e.getMessage());
    		...
    		if (count < 1) {
				return new ArrayList<RegularBid>();
			}
    		
    		pageBean.totalCount = count;
    		
    		query = em.createNativeQuery(sql.toString(), RegularBid.class);
    		...
    		try {
				...
				Logger.error("借款标管理->定期标列表：" + e.getMessage());
    	 ...
    	 * @return
    	 */
    	public static List<RecevingInvest> queryRegularAudit(int noPage, PageBean<RecevingInvest> pageBean, ErrorInfo error , String ... str){
			String[] BID_SEARCH = {" AND (`u`.`name` LIKE ? OR concat(`e`.`_value`,cast(`b`.`id` AS CHAR charset utf8)) LIKE ?)",
					" AND concat(`e`.`_value`,cast(`b`.`id` AS CHAR charset utf8)) LIKE ?", "", " AND `u`.`name` LIKE ?" };
    		...
    		sqlCount.append("SELECT COUNT(1) FROM  t_invests  i  LEFT JOIN  t_bids  b  ON  b.`id` = i.`bid_id`  LEFT JOIN  t_users  u  ON  i.`user_id` = u.`id`  WHERE   b.`is_regular`=TRUE  AND  b.`is_regular`=true ");
    		sql.append("   SELECT i.`id` AS `id`, b.`title` AS `title` ,b.`amount` AS `bidAmount`,b.`loan_schedule` AS `loan_schedule`, b.`apr` AS `apr`, u.`name` AS `name`, i.`amount` AS `amount` , i.`redeem_audit_id` AS `redeem_audit_id`, i.`redeem_time` AS `redeem_time` , i.`redeem_status` AS `redeem_status` "+
	    				 " FROM  t_invests  i  LEFT JOIN  t_bids  b  ON  b.`id` = i.`bid_id`  LEFT JOIN  t_users  u  ON  i.`user_id` = u.`id`  WHERE   b.`is_regular`=TRUE  AND   b.`is_regular`=true order by i.`redeem_time` desc");
    		
            //str[] ={userId, condition, keyword, startDate, endDate, orderIndex, orderStatus}   		
    		/* 条件 */
    		/*if (str.length > 1 && NumberUtil.isNumericInt(str[1])) {
				int c = Integer.parseInt(str[1]);
...
*/
    		...
    		try {
				...
				Logger.error("借款标管理->定期标赎回审核列表,查询总记录时：" + e.getMessage());
    		...
    		if (count < 1) {
				return new ArrayList<RecevingInvest>();
			}
    		
    		pageBean.totalCount = count;
    		
    		query = em.createNativeQuery(sql.toString(), RecevingInvest.class);
    		...
    		try {
				...
				Logger.error("借款标管理->定期标赎回审核列表：" + e.getMessage());
