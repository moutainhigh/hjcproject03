...
public class TemplateSms implements Serializable{
	...
	public static void dealSmsTask() {
		...
		//查询所有未发送的短信
		try {
			sendings = t_system_mobile_sms_sending.find("is_sent = 0 order by id desc ").fetch(Constants.JOB_MSG_AMOUNT);
		} catch (Exception e) {
		...
		for(t_system_mobile_sms_sending sending : sendings) {
			...
			try {
				...
				if(backstageSet.is_jwconfirm==1){
					SMSUtil.jwsendSMS(sending.mobile,sending.body);
				}else{
					SMSUtil.sendSMS(sending.mobile, sending.body,error); 
				}
			...
			//无论短信发送成功或失败，短信只发送一次
			try {
				em.createNativeQuery("update t_system_mobile_sms_sending set is_sent = 1 where id = ?")
				.setParameter(1, sending.id)
				.executeUpdate();
			} catch (Exception e) {
				Logger.error("定时发送短信时（更新发送状态is_sent=1）："+e.getMessage());
			}
		}
	}
