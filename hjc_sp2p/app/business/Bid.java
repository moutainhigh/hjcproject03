...
public class Bid implements Serializable{
	...
	public boolean isRegular; // 是否定期标
	 ...
	 * 根据标的id得到标的名称
	 */
	public String getTitle(long bdid){
		t_bids tb = new t_bids();
		String sql = "from t_bids where id="+bdid;
		Query query = JPA.em().createQuery(sql);
		List result = query.getResultList();
		for(int i =0 ;i<result.size();i++){
			tb = (t_bids)result.get(i);
		}
		String name = tb.getTitle();
		return name;
	}
	/**
	 * 获取加密ID
	...
	public void setId(long id) {
		...
		if(this.auditBid){
			...
			if(tbid.is_regular){
				this.isRegular = tbid.is_regular; // 是否活标
				this.loanAmount = tbid.loan_amount;
				this.originalAmount = tbid.original_amount;
			}
			return;
		}
		...
		this.originalAmount = tbid.original_amount; // 借款原始金额
		this.loanAmount = tbid.loan_amount; // 定期标已放款金额
	 ...
	 * @return -1:失败;  >0:成功;
	 */
	public  int addBidLoadAmount( double money){
		String hql = "update t_bids set loan_amount = loan_amount + ? where id = ?";
		Query query = JPA.em().createQuery(hql);
		...
		try {
			...
			JPA.setRollbackOnly();
			Logger.error("增加定期标的已放款金额时，" + e.getMessage());
	...
	* 每天给理财人返息 
	*/ 
	public static void returnDayRateToInvestUser() {

		List<Long> ids = null;
		String hql = "select id from t_bids where status = ? and now() <= repayment_time ";

		try {
			ids = t_bids.find(hql, Constants.BID_REPAYMENT).fetch();
		} catch (Exception e) {

			Logger.info("##自动返息->出错:##" + e.getMessage());
		...
		for (Long id : ids) {
			try {
				...
				//借款人提前还款
                if(StringUtils.isNotBlank(bid.recentRepayTime+"") 
                		&& bid.recentRepayTime.before(bid.repaymentTime)){
                	return;
                }
                
				List<Long> investIds = null;
				try {
					Double dayRate = Double.valueOf(bid.apr * 0.01)/365.0;// 通过年利率得到日利率
					// 查询当前标的所有理财人
					investIds = t_invests.find("select id from t_invests where bid_id = ? and transfer_status in (0,1) and redeem_status in (0,-1 , 1) ",id).fetch();
					for (Long investId : investIds) {
						// 遍历理财人投资记录返回当天利息
						invest = new Invest();
						invest.id = investId;
						boolean flag = bid.repaymentTime.before(new Date());
						long rateUser = invest.userId;
						
						//2015-11-27  2015-11-27
						if(DateUtil.dateToString1(new Date()).equals(DateUtil.dateToString1(bid.repaymentTime))){
							
							//Date rateDeadLine = Invest.querryDeadLineTime(investId,
							//		id, error);
							//long days = DateUtil.diffDays2(bid.repaymentTime,rateDeadLine) + 1;//由于当天放款 第二天才放息Arith.div(Arith.mul(Arith.mul(invest.amount, dayRate), days),1, 2);

							t_bill_invests  bill = t_bill_invests.find(" invest_id =? ", investId).first();
							
							double lastRate = bill.receive_interest - bill.real_receive_interest;
							
							int rows = DealDetail.addUserFund(rateUser,
									lastRate);

							if (rows < 1) {
								error.code = -1;
								error.msg = "理财人最后一天返息增加返息资金失败!";
								JPA.setRollbackOnly();
							...
							// 及时查询用户最新状态
							detail = DealDetail.queryUserFund(rateUser,
									error);

							DealDetail dealDetail = new DealDetail(
									rateUser, DealType.ADD_LAST_RATE,
									lastRate, id,
									detail.get("user_amount"),
									detail.get("freeze"), 0.00, "获得标的编号为J"
											+ bid.id + "剩余返息金额");

							dealDetail.addDealDetail(error);
							saveRateRecord(id,invest.userId,lastRate,Constants.DESCRIPTION_LAST,error);
							if(error.code<0){
								JPA.setRollbackOnly();
								error.msg="更新返息记录出错！";
							}
							Bill.updateInvestBillRate(lastRate,invest,error); 
							
							continue;
						}
						
						if (!flag) {
							double personDayRate =Arith.div(Arith.mul(Arith.mul(invest.amount, dayRate), Constants.ONE), Constants.ONE, Constants.TWO);
							...
							Map<String, Double> detail = null;
							// 及时查询用户最新状态
							detail = DealDetail.queryUserFund(invest.userId,
									error);

							DealDetail dealDetail = new DealDetail(
									rateUser, DealType.ADD_DAY_RATE,
									personDayRate, id,
									detail.get("user_amount"),
									detail.get("freeze"), 0.00, "获得标的编号为J"
											+ bid.id + "返息金额");
							...
							dealDetail.addDealDetail(error);
							saveRateRecord(id,rateUser,personDayRate,Constants.DESCRIPTION,error);
							if(error.code<0){
								JPA.setRollbackOnly();
								error.msg="更新返息记录出错！";
							}
							Bill.updateInvestBillRate(personDayRate,invest,error);
							
						}
						
					
					}
			   Logger.info("标自动返息正常关闭，bid_id = %s ", id);
				} catch (Exception e) {
					Logger.info("标自动返息出错:" + e.getMessage());
				} finally {
				...
				Logger.info("自动返息出错:" + e1.getMessage());
			}
	...
	public static void checkBidIsFlow(){
		List<Long> ids = null;
        String hql = "select id from t_bids where status in (?, ?) and is_regular=? and now() > invest_expire_time";
		
		try {
			ids = t_bids.find(hql, Constants.BID_ADVANCE_LOAN, Constants.BID_FUNDRAISE ,Constants.IS_NOT_REGULAR).fetch(); 
		} catch (Exception e) {
		...
		JPAPlugin.startTx(false);
		hql = "select id from t_bids where is_regular =? and status in (?, ?) and now() >= invest_expire_time and has_invested_amount < amount";

		try {
			ids = t_bids.find(hql, Constants.IS_REGULAR,
					Constants.BID_ADVANCE_LOAN, Constants.BID_FUNDRAISE)
					.fetch();
		} catch (Exception e) {

			Logger.error("定期标->检查是否到期满标时:" + e.getMessage());
		...
		for (Long id : ids) {
			...
			try {
				...
				if (bid.hasInvestedAmount < 1) {
					...
					Query query = JPA.em().createQuery(sql);
					query.setParameter(1, Constants.BID_FLOW); // 标的状态
					query.setParameter(2, bid.id); // 标的ID

					try {
						row = query.executeUpdate();

					} catch (Exception e) {
						Logger.error("定期标->流标:" + e.getMessage());
						JPA.setRollbackOnly();

						error.code = -1;
						error.msg = "定期标->流标，修改标的信息，数据库异常";
						return;
					} finally {
						Logger.info("定期标>定时任务>到期流标事务正常关闭，id = %s ", id);

					}

					if (row < 1) {
						error.code = -2;
						error.msg = "定期标>定时任务>到期满标 修改定期标的信息失败!";
						JPA.setRollbackOnly();
					...
					/* 算出保证金 金额*比例/100 */
					bid.bail = Arith.mul(bid.amount,
							Arith.div(bid.product.bailScale, 100, 10));

					if (bid.bail < 0) {
						error.code = -3;
						error.msg = "定期标>定时任务>到期满标 算保证金的时候出现错误!";

						return;
					}
					/* 得到服务费 */
					bid.serviceFees = ServiceFee.loanServiceFee(
							bid.hasInvestedAmount, bid.period, bid.periodUnit,
							error);

					if (bid.serviceFees < 0) {
						error.code = -3;
						error.msg = "定期标>定时任务>到期满标 获取系统管理费失败!";
					...
					String sql = "update t_bids set amount = ?, loan_schedule = ?, real_invest_expire_time = ?, service_fees = ?, bail=?  where id=?";

					Query query = JPA.em().createQuery(sql);
					query.setParameter(1, bid.hasInvestedAmount); // 借款总额
					query.setParameter(2, 100.00); // 借款进度改为100%
					query.setParameter(3, new Date()); // 实际满标时间
					query.setParameter(4, bid.serviceFees); // 更改借款管理费
					query.setParameter(5, bid.bail); // 更改保证金
					query.setParameter(6, bid.id); // 标的ID

					try {
						row = query.executeUpdate();

					} catch (Exception e) {
						Logger.error("定期标>定时任务>到期满标 修改状态出错:" + e.getMessage());
						JPA.setRollbackOnly();

						error.code = -1;
						error.msg = "定期标>定时任务>到期满标，修改标的信息，数据库异常";
				...
				Logger.error("定期标>定时任务>到期满标：" + e.getMessage());
			} finally {
				
				Logger.info("定期标>定时任务>到期满标事务正常关闭，id = %s ", id);
				JPAPlugin.closeTx(false);
...
//		tbid.loan_purpose_id = this.purpose.id; // 借款用途
		...
		if (this.agencyId > 0) {
			...
			tbid.is_quality = this.isQuality; 
		}
		
		/* 判断是否活标发布 */
		if(this.isRegular){
			tbid.original_amount = this.amount;
			tbid.is_quality = this.isQuality; 
		}
	 ...
	 * @param error
	 */
	public long afterCreateBid(t_bids tbid, String bidNo,String photos,int client, long supervisorId, ErrorInfo error) {
		error.clear();
		...
		if(!StringUtils.isBlank(photos)){
			 t_bid_imagefile_name  image = null;
			  String [] bid_photos = photos.split(",");
			  
			  //遍历图片并保存
			  for(String bid_photo : bid_photos){
				  image = new t_bid_imagefile_name(); 
					   image.bid_id = bid.id;
			           image.image_file_name = bid_photo;
			           image.upload_time = new Date();
			           try{
			        	   image.save();
			           }catch(Exception e){
			        	    Logger.error("##发标保存图片出错[s%]##",e);
			        	    error.code = -1;
							error.msg = "发标保存图片出错!";
							JPA.setRollbackOnly();
			           }
	...
	private void ensureStatusByMechanism(ErrorInfo error) {
		/* 后台发标 */
		if (this.agencyId > 0 || this.isRegular) {
			Logger.info("---------------------根据用户上传资料判断显示为提前借款还是借款中------------------------");
			/**
...
/*		if(sms.status){
	 ...
	 * 定期标借款中->提前结束 
	 */
	public void regularStopInAdvance(ErrorInfo error){
		error.clear();
		int row = 0;
		/* 算出保证金    金额*比例/100 */
		this.bail = Arith.mul(this.hasInvestedAmount, Arith.div(this.product.bailScale, 100, 10));
		...
		/* 得到服务费 */
		this.serviceFees = ServiceFee.loanServiceFee(this.hasInvestedAmount, this.period, this.periodUnit, error);
		...
		/* 3.修改状态 */
		String hql = "update t_bids set amount=?, real_invest_expire_time=?, status=? ,loan_schedule=?, bail=? ,service_fees=?  where id=? and user_id=?";

		Query query = JPA.em().createQuery(hql);
		query.setParameter(1, this.hasInvestedAmount);// 审核人
		query.setParameter(2, new Date());// 实际满标时间
		query.setParameter(3, Constants.BID_EAIT_LOAN);// 审核状态为提前结束
		query.setParameter(4, 100.00);// 进度为100 
		query.setParameter(5, this.bail);// 保证金
		query.setParameter(6, this.serviceFees);// 服务费
		query.setParameter(7, this.id); // 标ID 
		query.setParameter(8, this.userId); // 用户ID

		try {
			row = query.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("定期标->借款标->提前结束:" + e.getMessage());
			error.msg = "修改状态失败!";
			JPA.setRollbackOnly();
		...
		/* 添加事件 */
		DealDetail.supervisorEvent(this.allocationSupervisorId, SupervisorEvent.AUDIT_BID, "审核标：定期借款标->提前结束", error);
		
		if(error.code < 0){
			error.msg = "添加定期借款标->提前结束事件失败!";
			JPA.setRollbackOnly();
		...
		email.setId(Templets.E_REGULAR_STOP_SUCCESS);
		station.setId(Templets.M_REGULAR_STOP_SUCCESS);
		...
		if(station.status){
			...
			content = content.replace("date", DateUtil.dateToString((new Date())));
			content = content.replace("title", this.title);
			content = content.replace("money", this.amount+"");
			content = content.replace("fee", this.serviceFees+"");
		...
		error.code = 1;
		error.msg = "提前结束成功,已将标置为[待放款]!";
	}
	
	/**
	 * 定期标借款中->待放款 （及时放款审核）
	 */
	public void regularFundraiseToLoan(ErrorInfo error){
		error.code = -1;
		int row = 0;
		
		/* 1.确定当前bid的状态 */
		if(Constants.BID_FUNDRAISE != this.status){
			error.msg = "定期标状态不对，非法审核!";
			
			return;
		}
		
		if(this.hasInvestedAmount - this.loanAmount < 1){
			error.msg = "该定期标近期没有可放款金额，审核失败!";
		...
		try {
			...
			e.printStackTrace();
			Logger.error("定期标->借款标->待放款:" + e.getMessage());
			error.msg = "修改状态失败!";
			JPA.setRollbackOnly();
...
//		}
		...
		/* 5.添加事件 */
		DealDetail.supervisorEvent(this.allocationSupervisorId, SupervisorEvent.AUDIT_BID, "审核标：定期借款标->待放款", error);
		...
		TemplateStation station = new TemplateStation();
		TemplateEmail email = new TemplateEmail();
		email.setId(Templets.E_REGULAR_AUDIT_SUCCESS);
		station.setId(Templets.M_REGULAR_AUDIT_SUCCESS);
	 ...
	 * 定期标 借款中->放款不通过
	 */
	public void fundraiseRegularToLoanNotThrough(ErrorInfo error) {
		...
		/* 2.修改状态 */
		String hql = "update t_bids set allocation_supervisor_id=?, audit_time=? where id=? and user_id=? and status=?";

		Query query = JPA.em().createQuery(hql);
		query.setParameter(1, this.allocationSupervisorId);// 审核人
		query.setParameter(2, new Date());// 审核时间
//		query.setParameter(3, Constants.BID_FUNDRAISE);// 审核状态为放款不通过
		query.setParameter(3, this.id); // 标ID
		query.setParameter(4, this.userId); // 用户ID
		query.setParameter(5, Constants.BID_FUNDRAISE); // 当前状态必须为借款中

		try {
			row = query.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("定期标->借款中->放款不通过:" + e.getMessage());
			error.msg = "修改状态失败!";
			JPA.setRollbackOnly();
		...
		/* 6.添加事件 */
		DealDetail.supervisorEvent(this.allocationSupervisorId, SupervisorEvent.AUDIT_BID, "审核标：定期标->放款不通过", error);
	 ...
	 * 定期标->放款
	 */
	public void eaitRegularLoanToRepayment(ErrorInfo error){
		error.code = -1;
		...
		doEaitRegularLoanToRepayment(error);
	}  
	
	public void doEaitRegularLoanToRepayment(ErrorInfo error){
		
		error.code = -1;
		int row = 0;
		boolean isNotLastLoan = this.hasInvestedAmount < this.amount;
		...
		if(isNotLastLoan){ 
			...
			try {
				...
				e.printStackTrace();
				Logger.error("定期标->待放款->借款中:" + e.getMessage());
				error.msg = "定期标->待放款->借款中 修改状态失败!";
				JPA.setRollbackOnly();
			...
			try {
				...
				e.printStackTrace();
				Logger.error("定期标->待放款->还款中:" + e.getMessage());
				error.msg = "定期标->待放款->还款中 修改状态失败!";
				JPA.setRollbackOnly();
		...
		double user_amount =0; //及时查询的用户账户余额
		double freeze = 0;     //及时查询的用户账户冻结资金
		double receive_amount = 0;//及时查询的用户增加的资金
		double nowAmount = 0;
		if(this.hasInvestedAmount != this.loanAmount){//多次放款过程中先放款 再提前结束出现错误
			
			//把本次放款金额打给用户
			nowAmount = this.hasInvestedAmount - this.loanAmount; 
			int rows = DealDetail.addUserFund(this.userId, nowAmount);

			if (rows < 1) {
				error.msg = "定期标增加用户资金失败!";
				JPA.setRollbackOnly();
			...
			//标的本次放款金额更改
			rows = addBidLoadAmount(nowAmount);
			if (rows < 1) {
				error.msg = "定期标更新标的已放款资金失败!";
				JPA.setRollbackOnly();
			...
			dealDetail = new DealDetail(this.userId, DealType.ADD_LOAN_FUND,
					nowAmount, this.id, detail.get("user_amount"), detail.get("freeze"),
					detail.get("receive_amount"), "获得定期标借款金额");

			dealDetail.addDealDetail(error);
			
			if (error.code < 0) {
				error.msg = "定期标打钱给用户、添加交易记录失败!";
				JPA.setRollbackOnly();
			...
			List<Invest> invests = Invest.queryRegularInvestUser(this.id); // 获得该标的最近所投的投资者以及金额

			if (null == invests || invests.size() == 0) {
				error.msg = "定期标获得用户对标所投的金额有误!";
				JPA.setRollbackOnly();
			...
			//更改投资表定期标已经放款的状态
			String loan_sql ="update t_invests set loan_bid_status =? where id=?  ";
			for(int i =0 ;i<invests.size();i++){
				Query query = JPA.em().createQuery(loan_sql);
				query.setParameter(1, 1);// 审核人
				query.setParameter(2, invests.get(i).id);// 审核时间
				try {
					row = query.executeUpdate();
				} catch (Exception e) {
					e.printStackTrace();
		...
		if(!isNotLastLoan){
			...
			//秒还  非秒还
			if(this.isSecBid)
				summary = "定期标放款、解冻秒还借款冻结保证金";
			else
				summary = "定期标放款、解冻非秒还借款冻结保证金";
			...
			if(null == detail || detail.size() == 0){
			 ...
			 receive_amount = detail.get("receive_amount");
			/* 添加交易记录 */
			...
			dealDetail = new DealDetail(this.userId, DealType.CHARGE_LOAN_SERVER_FEE,
					...
					detail.get("receive_amount"), "支付定期标借款管理费");

			dealDetail.addDealDetail(error);
			
			if (error.code < 0) {
				error.msg = "定期标扣服务费、添加交易记录失败!";
				JPA.setRollbackOnly();
			...
			dealDetail.addPlatformDetail(DealType.LOAN_FEE, this.id, this.userId,
					-1, DealType.ACCOUNT, this.serviceFees, 1, "支付定期标借款管理费",
					error);
			
			if (error.code < 0) {
				error.msg = "定期标添加风险金失败!";
				JPA.setRollbackOnly();
			...
			/* 生成账单 */
			row = new Bill().addBill(this, nowAmount ,error);
			...
			Query query1 = JPA.em().createQuery(sql).setParameter(1, this.recentRepayTime).setParameter(2, this.id);
			
			try {
				row = query1.executeUpdate();
			} catch (Exception e) {
				Logger.error("定期标待放款（最后一次放款）->还款中：" + e.getMessage());
				error.msg = "修改定期标的还款日期失败！";
				JPA.setRollbackOnly();
			...
			long investUserId;
			String s="select user_id from t_invests where bid_id=?";
			list=JPA.em().createQuery(s).setParameter(1, this.id).getResultList();
			for (int i=0;i<list.size();i++){
				investUserId= (Long) list.get(i);
				/* 添加理财人CPS推广费 */
				User.rewardCPS(investUserId, this.serviceFees, this.id, error); //只插入cps奖励记录，真实业务逻辑，在线程中完成
			}
			...
			/* 添加事件 */
			DealDetail.supervisorEvent(this.allocationSupervisorId, SupervisorEvent.REPAYMENT_FUND, "定期标：待放款->还款中", error);
			
			if(error.code < 0){
				error.msg = "添加生成定期标借款合同事件失败!";
				JPA.setRollbackOnly();
			...
			email.setId(Templets.E_REGULAR_LAST_RELEASE);
			station.setId(Templets.M_REGULAR_FINANC_RELEASE);
			...
			email.setId(Templets.E_REGULAR_RELEASE);
			station.setId(Templets.M_REGULAR_RELEASE);
			sms.setId(Templets.S_REGULAR_RELEASE);
			...
			if(station.status){
				...
				content = content.replace("date", DateUtil.dateToString((new Date())));
				content = content.replace("title", this.title);
				content = content.replace("money", nowAmount+"");
			...
			//尊敬的userName: 您申请的编号bidId借款标已成功放款，借款金额amount元，扣除管理费0元
			if(sms.status && StringUtils.isNotBlank(this.user.mobile)){
				BackstageSet backstageSet = BackstageSet.getCurrentBackstageSet();
				content = sms.content;
				content = content.replace("userName", this.user.name);
				content = content.replace("bidId",backstageSet.loanNumber+this.id);
				content = content.replace("amount",DataUtil.formatString(this.amount)); 
				content = content.replace("serviceFees",DataUtil.formatString(0.00));
				content = content.replace("money",DataUtil.formatString(this.hasInvestedAmount - this.loanAmount));//到账金额
				sms.addSmsTask(this.user.mobile, content);
			}
			error.code = 1;
			error.msg = "定期标放款成功,已将标置为[借款中]!";
	...
	public void doEaitLoanToRepayment(ErrorInfo error){
		...
		/* 生成账单 */
		row = new Bill().addBill(this, 0.00,error);
...
//			content = content.replace("rate",DataUtil.formatString(serviceFees));
...
//		}
		...
		if (isReleaseSign) {
			...
			double deductMoney = dataMap.get("award");
			this.serviceFees = dataMap.get("bid_fee");
			double dayRate = Double.valueOf(this.apr * 0.01)/365.0;//通过年利率得日利率
		 	double monPayInterest = Arith.div(Arith.mul(Arith.mul(this.amount, dayRate), this.period), 1, 2);//天标的总利息  从借款第一天开始

			this.amount = Arith.sub(Arith.sub(this.amount, deductMoney), this.serviceFees+monPayInterest);
	...
	private void investUserAmountProcess(ErrorInfo error, List<Invest> invests ){
		...
		for (int i = 0; i < len; i++) {
			...
			dealDetail = new DealDetail(invest.investUserId, DealType.CHARGE_INVEST_FUND,
					...
					detail.get("receive_amount"), "扣除投标金额");
			...
			if(station.status){
				...
				content = content.replace("date", DateUtil.dateToString((new Date()))); 
				content = content.replace("title", this.title); 
			    content = content.replace("rebackAmount",  DataUtil.formatString(invest.investAmount)); 
...
//	}
	...
	private void returnInvestUserFund(String summary, ErrorInfo error) {
		...
		for (Invest invest : invests) {
			...
			if(this.title.contains("体验")){
				Bbin.returnBbin(invest.investUserId, invest.investAmount, error);
			}else{
	...
	public int countUpId; // 上个ID数
	public int countNextId; // 下个ID数 
	...
	public long getUpId() {
		if(0 == this.upId) {
			...
			case 600: sql += " and is_regular = 1"; break;
			}
	...
	public long getNextId() {
		if(0 == this.nextId) { 
			...
			case 503: sql = sql3; sql += " and b.status in (-3)"; break; //503.本金垫付理财账单列表。
			case 504: sql = sql2; sql += " and a.status in(4, 5) and b.overdue_mark = -3"; break; //504.坏账借款标列表。
			case 600: sql += " and is_regular = 1"; break;//600.定期标列表。
			}
	...
	public int getCountUpId() {
		if(0 == this.countUpId) {
			...
			case 504: sql = sql2; sql += " and a.status in(4, 5) and b.overdue_mark = -3"; break; //504.坏账借款标列表。
			case 600: sql += " and is_regular = 1"; break; //600.定期标列表。
			case 700: sql += " and status in(1, 2) and amount > has_invested_amount and is_regular = 1"; break;//定期标借款中及时放款审核
			}
	...
	public int getCountNextId() {
		if(0 == this.countNextId) {
			...
			case 503: sql = sql3; sql += " and b.status in (-3)"; break; //503.本金垫付理财账单列表。
			case 504: sql = sql2; sql += " and a.status in(4, 5) and b.overdue_mark = -3"; break; //504.坏账借款标列表。
			case 600: sql += " and is_regular = 1"; break; //定期标详情
			case 700: sql += " and status in(1, 2) and amount > has_invested_amount and is_regular = 1"; break;//定期标借款中及时放款审核
			}
	...
	public static int queryBidStatus(long id) {
		 ...
		 * @return
		 */
		public static int saveRateRecord(long bid_id , long invest_userId,double rate, String des ,ErrorInfo error){
			error.clear();
			 t_return_rate_record  rateRcords = new t_return_rate_record();	 
				 rateRcords.bid_id = bid_id;
				 rateRcords.invest_user_id = invest_userId;
				 rateRcords.return_time = new Date();
				 rateRcords.description = des;
				 rateRcords.rate = rate;
			    try {
